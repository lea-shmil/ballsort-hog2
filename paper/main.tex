...
\pdfinfo{
/Title (Evaluating BFS Algorithms on Colored Balls in Colored Tubes)
/Author (Lea Galuk, Shahaf Harari)
}

\setcounter{secnumdepth}{0}

\title{Evaluating BFS Algorithms on Colored Balls in Colored Tubes \\[4pt]
...
\cite{althaus2025}. The organizing template is best-first search, in which
a node is repeatedly selected for expansion from the search frontier
...
compared.
% [FIX 12] Code link filled in (was a TODO in the draft).
The domain implementation, every algorithm compared here, the instance
generator, and the scripts that ran the sweep and produced every number in
this paper are available at
\url{https://github.com/lea-shmil/ballsort-hog2}, tagged
\texttt{results-2026-08-02}. The Code and Data Availability section below
maps each result to the file that produced it.
...
NP-completeness of finding a solution; they also identify special cases
solvable in polynomial time.
% [FIX 11] The draft attributed two specific restrictions to Ito et al. ("even
% when restricted to two colors" and "bins of capacity two") that the published
% abstract does not state. Both removed. If they appear in the paper body,
% reinstate them with the theorem number.
...
space grows: expansions under $h_{DFVS}$ are almost independent of $N$ across
the whole core grid, while the uninformed searches track $N$ directly.
...
axes are logarithmic. The searches guided by $h_{DFVS}$ stay nearly flat while
$N$ grows by seven orders of magnitude, whereas the uninformed searches climb
with $N$ and stop at the cell where their budget runs out. Hollow markers mark
...
of 1.0017 and expansions falling to a median of 92. Greedy best-first
...
\texttt{wastar-1.1} & 1.0017 & 1.050 & 627/660 (95\%) & 92 \\
...
% =========================================================================
% [FIX 12] New section: concrete pointer to the code behind every number.
\section{Code and Data Availability}

Everything reported here is reproducible from
\url{https://github.com/lea-shmil/ballsort-hog2}. The dataset behind
Tables~\ref{tab:coverage} to~\ref{tab:failure} is tagged
\texttt{results-2026-08-02}, with HOG2 (\url{https://github.com/MovingAILab/hog2},
branch \texttt{PDB-refactor}) vendored as a submodule and pinned at commit
\texttt{af9d42d0}. Table~\ref{tab:artifacts} maps each component of the
study to the file that implements it.

\begin{table}[t]
\centering
\footnotesize
\caption{Where each part of this study lives in the repository.}
\label{tab:artifacts}
\begin{tabular}{p{3.1cm}p{4.3cm}}
\hline
Component & File \\
\hline
Domain: state, moves, goal test, perfect hash, $h_{mis}$ &
\texttt{environments/BallSort.h} \\
$h_{DFVS}$ and its exact minimum-DFVS solver &
\texttt{include/dfvs\_bound.h}, \texttt{include/dfvs\_exact.h} \\
Frontier search and IDDFS (our implementations) &
\texttt{include/frontier\_bfs.h}, \texttt{include/iddfs.h} \\
Algorithm 1 of Althaus et al. &
\texttt{include/rscbt.h} \\
Instance generation, grid, difficulty tiers &
\texttt{include/instance\_gen.h}, \texttt{src/generate\_instances.cpp} \\
One algorithm-instance run, one CSV row &
\texttt{src/run\_experiment.cpp} \\
Domain, hash and heuristic verification &
\texttt{src/main.cpp}, \texttt{src/test\_dfvs.cpp} \\
Sweep construction and submission &
\texttt{slurm/build\_tasklist.py}, \texttt{slurm/submit\_sweep.sh},
\texttt{slurm/experiment\_sweep.sbatch} \\
Aggregation and the correctness contract &
\texttt{slurm/aggregate\_results.py} \\
Figures~\ref{fig:coverage} and~\ref{fig:expansions} &
\texttt{slurm/plot\_results.py} \\
\hline
\end{tabular}
\end{table}

Three commands regenerate the instance set, run the sweep, and produce the
results:

{\small
\begin{verbatim}
slurm/build_tasklist.py --grid core \
    --seeds 20250726,20250727,20250728
...
\noindent
The last step writes \texttt{results/all\_runs.csv} (one row per algorithm,
instance and seed, with all recorded metrics),
\texttt{results/per\_cell.csv} (the per-cell medians and coverage), and
\texttt{results/contract.txt} (the correctness cross-check reported above);
it exits non-zero if any part of the contract fails. Every number in
Tables~\ref{tab:coverage} to~\ref{tab:failure} is derived from those two
CSV files.

Instance files are not distributed, because each is a deterministic
function of the tuple of seed, height, tube count and index, and is
regenerated rather than transferred. Solution lengths, nodes expanded,
nodes generated, iteration counts and root bounds reproduce bit-for-bit on
any machine, since every algorithm evaluated here is deterministic and
\texttt{rscbt-par} is deterministic in its thread count as well. Wall-clock
runtime, and anything derived from it, reproduces only approximately; each
row records the node and CPU model it ran on so that residual heterogeneity
is visible in the data rather than assumed away.
